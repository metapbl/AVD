...
- **Claude 의 신규 파일 작성 규칙**: 신규 파일이 등장하면 빈 파일 생성과 디렉터리 생성을 PowerShell 명령으로 함께 제시한다. 파일 내용은 별도 코드 블록으로 주고, 사용자가 VS Code 에서 붙여 넣어 저장한다 — 채팅 here-string 으로 본문까지 박는 동선은 들여쓰기·이스케이프 사고 위험이 커서 쓰지 않는다. 명령은 PowerShell 의 `New-Item` (디렉터리에는 `-ItemType Directory`, 파일에는 `-ItemType File`) 로 구성하고, 이미 존재할 가능성을 흡수하기 위해 `-Force` 를 붙인다. `New-Item -Force` 는 **기존 디렉터리는 보존**하지만 **기존 파일은 빈 파일로 덮어쓴다** — 후자의 경우 사용자에게 한 줄 경고를 곁들인다.
