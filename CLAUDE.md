...
본 문서와 `WORKLOG.md` 의 `##` / `###` 골격 헤더에는 1, 1.1 형식의 번호를 붙인다(깊이 2까지). 단, Changelog 의 날짜별 항목·단기 목록처럼 자주 추가·삭제되는 항목, ADR 의 고유 번호(ADR-001 등)는 별도 체계로 둔다. 참조 빈도가 높은 규칙 묶음은 `-` 대신 `1.` 순서 있는 리스트로 작성해 자연스러운 번호 참조를 가능하게 한다.
