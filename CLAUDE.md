...
- **Claude 의 코드 제시 규칙**: 코드를 제시할 때는 해당 파일에서의 들여쓰기 수준(모듈 최상위 / 클래스 메서드 / 중첩 블록)을 그대로 맞춰서 제시한다. 부분 발췌도 그대로 붙여넣어 동작하는 상태가 기본값이다. 약 300줄 미만의 파일이고 변경이 그 파일에 국한될 때는 파일 전체를 제시한다. 그보다 길거나 큰 파일에서 한 함수만 손댈 때는 해당 함수(또는 메서드) 단위로 부분 발췌한다. 한 줄짜리 수정은 예외로 둔다. 파일 전체를 보낼 때는 코드 블록 바깥에 이번 변경 지점을 한 문장으로 명시한다.
