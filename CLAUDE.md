...
- 의미 단위 커밋 후엔 `CHANGELOG.md` 의 맨 위에 항목 추가 — 단순/일반/결정 분량 한도, 분류 헷갈리면 사용자 확인, `CHANGELOG.md` / `WORKLOG.md` 만 건드린 메타 커밋은 제외.
- 큰 결정(아키텍처·라이브러리 선택·중대 버그 해결)은 `WORKLOG.md` 의 ADR 섹션에 기록.
- 커밋 작성자는 `metapbl <ggoyong2@gmail.com>` 으로 고정한다. 리포 로컬 `.git/config` 에 박아 두되, 전역 설정이 비면 도구가 자기 기본 이메일로 커밋해 버린 전례가 있으므로 push 전 `git log -1 --format="%an <%ae>"` 로 확인한다. 어긋났으면 `git commit --amend --reset-author --no-edit` 로 정정. (배경: `LESSONS.md` Git 그룹, 2026-07-29 `fbbccbf`)

## 8. Claude 응답 규칙

### 8.1. 소스 코드 참조

**로컬과 원격 중 어느 쪽이 최신인지 단정하지 않는다.** 이 프로젝트는 여러 환경(웹 채팅·Claude Code·GenSpark)에서 작업하므로 로컬이 앞설 때도, 다른 환경에서 push 해 원격이 앞설 때도 있다. 선후 관계는 추측이 아니라 `git status`·`git fetch`·`git log --oneline -3` 의 출력으로 확인한다. 파일을 어떻게 확보할지는 실행 환경에 따라 갈린다.

1. **리포지토리에 직접 접근 가능한 환경** (Claude Code 등 파일시스템 또는 인증된 git 접근이 있는 도구) — 작업 트리에서 파일을 직접 읽는다. 사용자에게 붙여넣기를 요청하지 않는다. 다만 읽기 전에 위 세 명령으로 원격과의 선후를 먼저 확인하고, 원격이 앞서 있으면 pull 을 안내한 뒤 패치를 설계한다.
2. **웹 채팅 환경** (claude.ai, 인증 없음) — 리포가 공개 상태면 11.1 의 raw URL 로 가져와 읽는다. 그쪽이 빠르다. 404·빈 응답·잘림이 나오거나 리포가 비공개면 즉시 포기하고 사용자에게 로컬본을 요청한다. 어떤 파일이 왜 필요한지 함께 밝힌다. 원격을 읽어 패치를 설계할 때는 로컬에 미push 변경분이 있을 가능성을 짧게 환기한다.

두 경우 공통으로, **가져오기 실패나 이상 응답으로부터 리포 상태를 추론하지 않는다.** 웹 채팅에서 원격을 못 읽었다는 사실은 그 파일이 원격에 없다는 뜻이 아니다. 비공개 전환·CDN 캐시·도구 오류가 모두 같은 모습으로 나타난다. "원격에 X 가 없다"·"로컬이 원격보다 앞서 있다" 같은 판단은 인증된 접근이나 사용자의 `git` 출력으로만 내린다.

가져온 내용이 사용자의 설명과 어긋날 때도 마찬가지다. 공개 시절 캐시가 남아 낡은 내용이 **성공 응답으로** 돌아오는 경우가 있으므로, 어긋나면 캐시를 의심하고 사용자에게 확인한다.
...
**리포가 비공개인 동안은 아래 raw URL 방식이 동작하지 않는다** (404). 그동안은 새 세션 첫 메시지에 로컬의 `CLAUDE.md` 와 `WORKLOG.md` 를 첨부하고, 작업 종류에 따라 `ADR.md`·`LESSONS.md`·`CHANGELOG.md` 를 함께 첨부한다. 세션 중 추가 파일이 필요해지면 Claude 가 요청하고 사용자가 로컬본을 준다 (8.1 참조). 아래는 리포를 다시 공개했을 때의 절차다.
