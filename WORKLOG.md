...
- **미사용 import 정리 — `main.py` 의 `from PySide6.QtCore import Qt`**
    - 동기: `main.py` 가 `Qt` 를 직접 참조하지 않는데 import 만 남아 있음 (faulthandler 제거 작업 중 발견). 린트 경고·가독성 부채.
    - 정책: 해당 줄 제거. 동작 변경 없음(`chore`). 제거 후 `python main.py` 정상 기동만 확인하면 충분.
    - 보류: 다른 모듈의 미사용 import 전수 점검은 Ruff `F401` 일괄 처리로 별도 묶을 수 있음 — 양이 쌓이면 한 번에.
