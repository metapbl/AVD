...
- **GaindB 라이선스 재정정 — Apache-2.0 → LGPL-2.1-or-later 재벤더링 (ADR-008·009 후속, 착수 대기)**
    ...
    - 작업 2-b (배포 방침 명문화, 같은 문서 손볼 때 함께): (1) **비영리 무료 공개** 방침을 루트 `NOTICE`·`README.md` 에 각 한 줄 — 영리 전환을 포기하고 GitHub 무료 오픈소스로 배포한다는 확정 결정(영리 시 `gaindb/` 공개 의무·소비자보호법 책임·권리자 분쟁 유인이 모두 커지므로). (2) `README.md` 에 이용 제한·면책 문구 — 사용자 책임 하 합법적 범위 사용, 사이트 약관·저작권 준수 고지, 무보증(LGPL·Apache 둘 다 무보증). (3) DRM 우회·유료 콘텐츠 접근을 돕는 기능은 도입하지 않는다는 방침을 `README.md` 또는 ADR-010 에 명기 — 3.4 의 쿠키 지원 보류가 그 첫 사례이며, 향후 도입 검토 시 이 방침이 판단 기준.
    ...
    - 빌드 접점(선반영): LGPL 은 사용자가 라이브러리를 자기 수정본으로 교체할 수 있을 것을 요구하므로, PyInstaller 빌드 시 `gaindb/` 를 exe 번들에서 제외하고 exe 옆 평문 `.py` 로 배치해야 한다(spec `excludes` + `datas`). PySide6(LGPLv3)도 Qt DLL 분리로 같은 요건을 충족하므로 **onedir** 구성이 두 건을 동시에 해결한다. 배포물에는 루트 `LICENSE`·`NOTICE`, `gaindb/` 라이선스 3파일, 신규 `THIRD_PARTY_LICENSES`(PySide6/Qt·yt-dlp(Unlicense)·requests·numpy·scipy·psutil 등) 동봉 필요. **소스 제공 의무는 GitHub Release 에 해당 태그의 소스 아카이브를 exe 와 함께 첨부하는 것으로 이행한다** — 리포를 나중에 비공개로 전환해도 이미 배포된 exe 에 대한 의무는 남으므로, 릴리스마다 소스를 남기는 것을 규칙으로 고정.
    - 검증: 재벤더링 후 `python main.py` 기동, MP3 다운로드 시 음량 정규화 정상 동작, 완료 라벨 dB 표시 확인. 코드 무변경이면 회귀 위험 낮음.
    - LGPL 준수 5항 대응표(배포 직전 점검용): (1) 고지 보존 — mp3gain 원저작자(Glen Sawyer) 고지를 `gaindb/NOTICE` 에, (2) 변경 표시 — 벤더링·수정 사실을 같은 파일에, (3) 전문 포함 — `gaindb/LICENSE` 에 LGPL-2.1 전문, (4) 소스 제공 — 릴리스에 해당 버전 소스 아카이브 첨부(위 빌드 접점), (5) 교체 가능성 — onedir 배치로 `gaindb/` 평문 `.py` 노출. 다섯 항목이 각각 어느 산출물에 대응하는지 확정해 두면 배포 시 누락 점검이 한 번에 끝난다.
...
- **GaindB 음량 정규화 — 문서 잔여 (ADR-008 후속)**
    - 완료: 벤더링본을 GaindB api 계층으로 교체(`39aeffe`), 새 api 연동·완료 라벨 dB/클리핑 표시(`86271f6`), 환경설정 접이식 슬라이더+입력창 UI(`d1d7135`), GaindB v2 안정본(`42b3df7`) 후 **v3 최종본 재벤더링(`b582bc9`)**. 클리핑 판정은 GaindB 본체가 `analyze_file` 의 `clip_state`("none"/"possible"/"definite")로 제공 → AVD 는 `definite` 만 클리핑으로 표시(강제 적용·표시Only 방침 유지). SpinBox 초안 폐기 후 `QLineEdit`+`QDoubleValidator`+`QSlider`(0.5dB, ×2 스케일) 조합으로 정착.
    ...
    - v5 재벤더링 **완료(2026-07-26)**: 상류 GaindB 가 <https://github.com/metapbl/GainDB>(0.5.0)로 공개됨. `gaindb/` 3파일 개정 반영 — `__init__.py`(버전/프로그램명 단일 진실원), `decode.py`(ffmpeg 자식 프로세스 `CREATE_NO_WINDOW` — Windows 콘솔 깜빡임 방지, AVD 직접 이득), `id3.py`(`M3G_ERR_*`→`ID3_ERR_*` 명명 정리, API 무변경). 트랙 모드 공개 API 시그니처 불변으로 `download_worker.py` 무수정. 라이선스 3파일 내용 동일. 공개 URL 을 루트 `NOTICE`·ADR-008 에 벤더링 출처로 기록.
...
- **ADR-008** GaindB(mp3gain 독립 구현, Apache-2.0) 통합 — MP3 다운로드 후 음량 정규화 (Accepted, 2026-07-10; UI·클리핑·실환경 검증 완료, 2026-07-24 v3 최종본·Apache-2.0 반영, 2026-07-26 v5(0.5.0) 재벤더링·상류 공개 URL 기록) — GaindB 벤더링으로 MP3 완료 직후 트랙 모드 음량 정규화(목표 89dB, 75.0~105.0). MP3 한정, 게인 실패해도 다운로드는 성공+실패 표시, MERGING·NORMALIZING 슬롯 해제. numpy/scipy(BSD) 추가. 접이식 슬라이더+입력창 UI·완료 라벨 dB/클리핑 표시 완료. 서브패키지 `gaindb/` 는 Apache-2.0(LICENSE·NOTICE·THIRD_PARTY_LICENSES 동봉). → [ADR.md#adr-008](./ADR.md#adr-008)
