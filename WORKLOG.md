...
- **LGPL 배포 산출물 정비 — PyInstaller onedir + 릴리스 소스 첨부 (ADR-010 후속, 착수 대기)**
    - 배경: 2026-08-08 재벤더링으로 `gaindb/` 가 LGPL-2.1-or-later 가 됐다(ADR-010). 문서·헤더·고지 쪽 준수는 끝났고, **남은 것은 배포물 형태**뿐 — 아직 배포(exe 빌드)를 하지 않았으므로 실제 의무 발생 전에 준비만 해 둔다.
    - 작업 1 (교체 가능성, LGPL §6): PyInstaller spec 에서 `gaindb/` 를 exe 번들에서 제외하고(`excludes`) exe 옆 평문 `.py` 로 배치(`datas`). 사용자가 자기 수정본으로 교체 가능해야 한다는 요건. PySide6(LGPLv3)도 Qt DLL 분리를 요구하므로 **onedir** 구성이 두 건을 동시에 해결한다(onefile 금지).
    - 작업 2 (소스 제공, LGPL §4): 릴리스마다 해당 태그의 소스 아카이브를 exe 와 함께 GitHub Release 에 첨부한다. 리포를 나중에 비공개로 전환해도 이미 배포한 exe 에 대한 의무는 남으므로 규칙으로 고정. 상류 `gaindb/` 소스는 <https://github.com/metapbl/GainDB> 에도 있으나, 벤더링 시점 사본을 릴리스에 남기는 편이 안전하다.
    - 작업 3 (동봉 파일): 배포물에 루트 `LICENSE`·`NOTICE`, `gaindb/LICENSE`·`gaindb/NOTICE`·`gaindb/THIRD_PARTY_LICENSES`, 그리고 AVD 자체 의존성용 `THIRD_PARTY_LICENSES`(PySide6/Qt LGPLv3 전문·yt-dlp(Unlicense)·requests·numpy·scipy·psutil 등) 신설·동봉. 현재 루트에는 `NOTICE` 만 있고 전문 모음 파일이 없다.
    - 점검표(배포 직전, LGPL 준수 5항): (1) 고지 보존 — mp3gain 원저작자(Glen Sawyer) 고지가 `gaindb/NOTICE` 에 ✅완료, (2) 변경 표시 — 벤더링·수정 사실 명시 ✅완료, (3) 전문 포함 — `gaindb/LICENSE` LGPL-2.1 전문 ✅완료, (4) 소스 제공 — 위 작업 2 (미착수), (5) 교체 가능성 — 위 작업 1 (미착수).
    - 보류: 장기적으로 mp3gain 참조 없는 독립 재작성 후 Apache-2.0 복귀는, 동일인이 이미 원본을 본 이상 clean-room 요건 충족이 어려워 현실성 낮음 — 당분간 LGPL 유지(ADR-010 대안 B).

- **벤더링 원본 보존 원칙 메모 (ADR-008·010 후속, 정보성)**
    - `gaindb/` 는 상류 무수정 복사가 원칙이다. 원본에 unused import 3건(`api.py` 의 `analyze_track`·`track_peak`, `tag.py` 의 `dataclasses.field`)이 남아 있으나 임의 수정하지 않는다 — 향후 upstream 재동기화 diff 를 최소화하기 위함. 연동부(AVD 자체 코드)는 ruff 통과. LGPL 재정정(0.5.1) 이후에도 같은 원칙 유지.

- **`requirements.txt` yt-dlp 핀 정책 — 의도적 저버전 유지 (자동 업데이트 리허설)**
    - 결정: `requirements.txt` 의 yt-dlp 핀을 최신으로 올리지 않고 저버전(현재 2026.3.17)으로 둔다. 근거 — `pip install -r requirements.txt` 가 항상 저버전을 설치하면, 앱 기동 시 자동 업데이트(비동기 모달 `37b6164`, ADR-007 계열)가 반드시 발동해 최신화한다. 즉 낮은 핀은 버그가 아니라 매 설치마다 "구버전 설치 → 자동 업데이트 발동 → 최신화" 경로를 실환경에서 리허설하게 만드는 장치다(2026-07-23 실기에서 정상 최신화 확인).
    - 함의: 향후 "yt-dlp 핀이 왜 낮지? 올려야 하나?" 라는 의문이 재등장하면 이 항목을 근거로 유지. 실제 다운로드 동작은 자동 업데이트 후 최신 yt-dlp 로 수행되므로 저버전 핀이 기능을 저해하지 않는다.

- **2026-07-09 작업분 Windows 실환경 검증** — 썸네일 게이트·다운로드 자동 재시도(ADR-007)·썸네일 폴백 체인·봇 차단 안내·yt-dlp 업데이트 비동기 모달(`37b6164`) 을 실기에서 확인. 절차서: [`docs/VERIFY_2026-07-09.md`](./docs/VERIFY_2026-07-09.md). 사용자 검증 완료 후 이 항목 삭제.
...
- **다운로드 일시정지/재개 (보류·재검토 대상)** — 2026-07-10 사용자가 "취소 대신 일시정지" 를 제안. 기술 판단: YouTube 다운로드 URL 은 시간 제한 토큰이 걸려 있어 일시정지 후 재개 시 대부분 URL 만료(403) → `.part` 이어받기 실패 → 사실상 처음부터 재시작이라 "일시정지" 라는 이름값을 못 한다. 진짜로 구현하려면 재개 시 `extract_info` 를 새로 돌려 새 URL 을 받고 `.part` 와의 바이트 오프셋 정합성(HTTP Range 재요청)을 맞춰야 함 — yt-dlp `continuedl` 동작·프래그먼트 재개 검증 필요. 도입 시 ADR 로. 지금은 진행 중 ✕ 숨김(`c26c0db`) 으로 취소 UX 혼란만 우선 해소.
...
- **쿠키(브라우저 로그인) 지원 — 봇 차단·로그인 전용 영상 대응**
    - 동기: 2026-07-09 실기에서 대량 다운로드 중 YouTube 가 "Sign in to confirm you're not a bot" (403) 로 차단. 이건 코드 버그가 아니라 YouTube 측 봇 방지로, "로그인한 사람의 요청"임을 증명하면 상당수 풀린다. 로그인 증명 = **쿠키**. 로그인 전용·연령제한·멤버십 영상도 같은 원리로 접근 가능.
    - 쿠키가 뭔가(사용자 설명 요약): 브라우저가 유튜브에 로그인하면 "이 사람은 로그인된 OOO" 이라는 작은 신분증 파일(쿠키) 을 브라우저가 보관한다. yt-dlp 에 그 신분증을 건네주면, yt-dlp 가 "로봇이 아니라 로그인한 사용자" 로 보여 봇 차단·로그인 벽을 통과한다.
    - yt-dlp 가 제공하는 두 방식:
        1. `--cookies-from-browser <브라우저>` — 설치된 브라우저(chrome/edge/firefox 등) 의 로그인 쿠키를 yt-dlp 가 자동으로 읽음. 사용자가 파일을 만들 필요 없음. 가장 간편. 단 브라우저가 쿠키 DB 를 잠그면(실행 중) 실패할 수 있음.
        2. `--cookies <cookies.txt>` — 사용자가 브라우저 확장(Get cookies.txt 등) 으로 내보낸 쿠키 파일을 지정. 브라우저와 독립적이라 안정적이나 사용자가 파일을 만들어야 함.
    - 구현 방향(초안, 미확정):
        1. 환경설정에 "브라우저 쿠키 사용" 옵션(끄기/chrome/edge/firefox…) + "쿠키 파일 경로" 옵션 추가. `ConfigManager` 에 저장.
        2. `core/downloader.py`·`core/info_fetcher.py` 의 `YoutubeDL` opts 에 `cookiesfrombrowser` / `cookiefile` 주입.
        3. **보안·프라이버시 주의**: 쿠키는 로그인 세션 그 자체 = 계정 접근 권한. 파일 유출 시 계정 탈취 위험. 앱이 쿠키를 복사·저장하지 않고 "경로만" 참조하도록 하고, 로그·백업에 절대 포함하지 않는다. 이 판단은 ADR 로 남길 것.
    - 상태: 사용자에게 개념 설명 완료(2026-07-09). 구현은 별도 세션. 봇 차단 안내(feat 완료) 가 임시 완화책이고, 이 기능이 근본 대응.
- **자동 업데이트 UX 실환경 검증** — `feat(ui) 37b6164` (업데이트 비동기화·모달) 를 Windows 실기에서 확인. 신버전 감지 → 모달 진행바 애니메이션 → 메인창 차단 → 완료 후 닫기. (앱 자체 자동 업데이트 `AppUpdater` 안전 갱신 검토는 별개로 유지.)
...
- **ADR-008** GaindB(mp3gain 파생 Python 재구현, LGPL-2.1-or-later) 통합 — MP3 다운로드 후 음량 정규화 (Accepted, 2026-07-10; UI·클리핑·실환경 검증 완료, 2026-07-24 v3 최종본 반영, 2026-07-26 v5(0.5.0) 재벤더링·상류 공개 URL 기록, **2026-08-08 라이선스 서술은 ADR-010 이 철회·대체**) — GaindB 벤더링으로 MP3 완료 직후 트랙 모드 음량 정규화(목표 89dB, 75.0~105.0). MP3 한정, 게인 실패해도 다운로드는 성공+실패 표시, MERGING·NORMALIZING 슬롯 해제. numpy/scipy(BSD) 추가. 접이식 슬라이더+입력창 UI·완료 라벨 dB/클리핑 표시 완료. 서브패키지 `gaindb/` 는 **LGPL-2.1-or-later**(LICENSE·NOTICE·THIRD_PARTY_LICENSES 동봉) — 본 ADR 의 "Apache-2.0"·"독립 구현" 서술은 ADR-010 참조. → [ADR.md#adr-008](./ADR.md#adr-008)

- **ADR-009** 프로젝트 라이선스 MIT → Apache-2.0 전환 (Accepted, 2026-07-24; 전환 동기 일부 소멸 — ADR-010) — 번들 서브패키지 `gaindb/` 가 Apache-2.0 이 되면서 본체(MIT)와 갈린 표기를 통일. 본체도 Apache-2.0 으로 전환(저작권자 `META PUBLIC`), 루트 `LICENSE`(전문)·`NOTICE`(서드파티 고지) 정비. Apache-2.0 의 명시적 특허 조항 보호를 프로젝트 전체로 확장. **주의**: ADR-010 으로 `gaindb/` 가 LGPL-2.1-or-later 가 되어 "서브패키지와 표기 통일" 동기는 소멸했고, 특허 조항 근거만 남아 본체 Apache-2.0 결정은 유지된다. → [ADR.md#adr-009](./ADR.md#adr-009)

- **ADR-010** GaindB 라이선스 재정정 — 서브패키지 `gaindb/` 를 LGPL-2.1-or-later 로 (Accepted, 2026-08-08; Supersedes ADR-008 의 라이선스 서술) — mp3gain(LGPL-2.1-or-later) 파생 확인으로 ADR-008 의 "독립 구현"·"clean-room" 서술을 철회하고, 상류 GaindB 0.5.1 을 LGPL 상태로 재벤더링. **폴더 경계 이중 라이선스** 확정 — 본체 Apache-2.0(`gaindb.api` 공개 함수만 호출 → LGPL-2.1 §5 "라이브러리를 사용하는 저작물"), 서브패키지만 LGPL-2.1-or-later. 코드 로직 무변경(AST 비교 확인)이라 공개 API 시그니처 불변·`download_worker.py` 무수정. LGPL 준수 5항(고지 보존·변경 표시·전문 포함·소스 제공·교체 가능성)을 산출물에 매핑, 배포 방침(비영리 무료 공개)·무보증·DRM 우회 금지 방침을 README 에 명문화. → [ADR.md#adr-010](./ADR.md#adr-010)
