...
- **자동 업데이트 UX 실환경 검증** — `feat(ui) 1fd5c95` (업데이트 비동기화·모달) 를 Windows 실기에서 확인. 신버전 감지 → 모달 진행바 애니메이션 → 메인창 차단 → 완료 후 닫기. (앱 자체 자동 업데이트 `AppUpdater` 안전 갱신 검토는 별개로 유지.)
