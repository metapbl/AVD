...
- **`info["thumbnail"]` 단일값(YouTube `maxresdefault.jpg`)은 최고화질 원본이 없는 영상에서 404**. 미리보기가 이 한 URL 만 시도하면 개인 업로드·가사영상 등에서 영영 빈다. 해법: `thumbnails` 리스트 상위 후보 + videoid 로 조립한 표준 URL(maxres→sd→`hqdefault`)을 후보 리스트로 넘겨 순차 폴백. `hqdefault` 는 모든 YouTube 영상에 존재하는 최후의 보루. (2026-07-09)
...
- **MP3 '주석(comment)' 만 깨지고 제목·아티스트는 멀쩡한 별개 증상이 있다**. FFmpeg 는 `-metadata comment=X` 를 표준 `COMM` 이 아니라 **비표준 `TXXX:comment` (사용자 정의 텍스트) 프레임에 UTF-16 으로** 쓴다. mutagen 으로 열면 UTF-16 자체는 정상인데, 한글 Windows 탐색기 '주석' 열·일부 플레이어가 이 비표준 프레임을 CP949 로 잘못 해석해 `?뱀떊怨쇱쓽…` mojibake 를 낸다. 제목/아티스트는 표준 `TIT2`/`TPE1` 프레임이라 영향 없음 — 그래서 "주석만" 깨진다. 근본 회피는 comment 프레임을 아예 안 만드는 것: `meta_comment` info 값을 **빈 문자열**로 두면 ffmpeg 가 comment 프레임 자체를 생성하지 않는다(실측). 영상 URL 은 `purl` 태그가 따로 보관하므로 정보 손실 없음. `-write_id3v1 1` 제거와는 다른 문제라 그 수정만으론 안 잡힘. (2026-07-10 실기 확인, ADR-004 부분 번복)
