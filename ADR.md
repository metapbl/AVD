...
- **상태**: Accepted (부분 — UI·클리핑 표시는 진행 중, WORKLOG 3 참조)
- **날짜**: 2026-07-10
- **관련 커밋**: `e1a8a26` `17c8933` `51a071f`
...
클리핑 판정은 GaindB 본체에서 먼저 정리 후 반영 예정(현 `apply_track_gain` 반환에 피크·클리핑 필드 없음). 방침은 "설정값 강제 적용, 클리핑 여부만 표시". 설정 UI 는 다이얼 방식 폐기·유틸리티 버튼형으로 재설계, 최소/최대값 텍스트 표기. 완료 라벨에 적용 dB 표시. 세부는 WORKLOG 3.
